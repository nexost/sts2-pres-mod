...
/// <summary>
/// Step 6 test mode "balance": one run played by a heuristic bot instead of AutoSlay's random, unkillable one, for any
/// character (--trump-character, default TRUMP). AutoSlay still walks the map, events, shops and screens; these
...
///   card reward : rarer first, Powers slightly preferred; with --trump-favor ID,ID,... those cards come first
...
/// Runs are at Ascension 0. With --trump-fullheal the character heals to full after each fight (same for every
...
public static partial class DevHarness
{
	...
	private static int _deportsThisCombat;
	...
	private static string CharacterArg => CommandLineHelper.GetValue("trump-character") ?? "TRUMP";
	...
	internal static async Task BalanceCombat(Rng random, CancellationToken ct)
	{
		...
		while (CombatManager.Instance.IsInProgress && turn < 60)
		{
			...
			for (int plays = 0; plays < 40; plays++)
			{
				...
				maxWall = Math.Max(maxWall, WallCmd.GetHeight(me.Creature));
			}
			if (me.PlayerCombatState?.Phase == PlayerTurnPhase.Play && CombatManager.Instance.IsInProgress)
			{
		...
		_balanceCombats.Add(new Dictionary<string, object>
		{
		...
		else if (CommandLineHelper.HasArg("trump-fullheal"))
		{
	...
	private sealed class FightView
	{
		...
		/// <summary>Whether this much damage from this card removes the enemy: a kill, or a Deport once it's under the line.</summary>
		public bool Kills(Creature enemy, double damage, CardModel? card = null)
		{
			...
			return card is IDeportingCard deporting && !DeportCmd.IsImmune(enemy)
				&& enemy.CurrentHp - Math.Max(0, damage - enemy.Block) <= (double)DeportCmd.LineHp(enemy, Me, deporting.DeportLineMultiplier);
		}
	}
	...
	private static double CardValue(CardModel card, FightView fight, Creature? target, int turn)
	{
		...
		value += BuildValue(fight, Get(CardVars.Build)) + Get("Summon") * 0.8;
		...
		value += Get("TariffPower") * 1.5 * spread + Get("Gold") * 0.15 - Get(PayGoldCard.GoldCostVar) * 0.04;
	...
	/// </summary>
	private static double BuildValue(FightView fight, double build)
	{
		if (build <= 0)
		{
			return 0;
	...
	{
		_favored ??= (CommandLineHelper.GetValue("trump-favor") ?? "").Split(',', StringSplitOptions.RemoveEmptyEntries).ToHashSet();
	...
	internal static void OnBalanceDeported(Creature _)
	{
		_deportsThisCombat++;
	}
	...
	private static void EndBalanceRun(string result)
	{
		...
		var summary = new Dictionary<string, object?>
		{
			...
			["seed"] = CommandLineHelper.GetValue("trump-seed"),
			["result"] = result,
