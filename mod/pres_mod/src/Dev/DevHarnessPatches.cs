...
/// <summary>
/// Test mode only: keep test saves in modded_prestest/ instead of modded/, so automated runs
/// (which abandon whatever run is in progress) can never touch real saves.
/// Also usable on its own with --pres-savedir &lt;name&gt; (the uninstaller's self-test points cleanup at test saves).
...
{
	private static string SaveDirName => CommandLineHelper.GetValue("pres-savedir") ?? "modded_prestest";

	public static bool Prepare() => DevHarness.Enabled || CommandLineHelper.HasArg("pres-savedir");
...
/// Right at that point, lock every character but the one under test, so the bot always plays it.
...
public static class TestOnlyModCharactersPatch
{
	...
	public static void Postfix(NCharacterSelectButton __instance)
	{
		...
		{
			// Only the character under test (--pres-character; balance runs may pick a base character like IRONCLAD).
			bool unlocked = __instance.Character.Id.Entry == DevHarness.TestCharacterId;
...
/// <summary>Test mode with --pres-mute: every master volume change becomes 0, including the un-mute on focus.</summary>
[HarmonyPatch(typeof(NAudioManager), nameof(NAudioManager.SetMasterVol))]
public static class TestMutePatch
{
	public static bool Prepare() => DevHarness.Enabled && CommandLineHelper.HasArg("pres-mute");
