...
/// <summary>
/// Launched by the uninstaller with --pres-cleanup: removes run saves and run history that reference this mod's
...
/// Every removed file is first copied to --pres-out/backup/. Then the game quits.
/// test.py cleansaves runs it on the test saves, with --pres-cleanup-ids for a character that was removed.
/// </summary>
public static class SaveCleanup
{
	public const string ArgMode = "pres-cleanup";
	...
	private static void Tick()
	{
		...
		_done = true;
		string outDir = CommandLineHelper.GetValue("pres-out") ?? ProjectSettings.GlobalizePath("user://pres_cleanup");
		var removed = new List<string>();
		var errors = new List<string>();
		try
		{
			ISaveStore store = Traverse.Create(SaveManager.Instance).Field("_saveStore").GetValue<ISaveStore>();
			// Plus --pres-cleanup-ids A,B: models no longer in the DLL (a character that was removed or renamed).
			string[] ids = typeof(ModEntry).Assembly.GetTypes()
				.Where(t => !t.IsAbstract && t.IsSubclassOf(typeof(AbstractModel)))
				.Select(t => ModelDb.GetId(t).ToString())
				.Concat((CommandLineHelper.GetValue("pres-cleanup-ids") ?? "").Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
				.Select(id => "\"" + id + "\"")
				.ToArray();
