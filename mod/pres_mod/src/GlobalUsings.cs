// Game namespaces nearly every card, power, relic and potion needs, so each model file only lists what's unusual.
// Only game namespaces here: a character's own namespaces are imported per file, so characters stay separate.
global using System;
